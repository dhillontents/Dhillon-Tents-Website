...
PUBLISH: Upload the contents of this dhillon-tents folder to your static web root. Keep every HTML, CSS and JavaScript file and the assets folder together. No build is required.
...
VERIFICATION: Desktop and mobile-width browser checks covered planner movement, form visibility, navigation and saved quotes. All tent sizes, table arrangements, geometry rules and quote replacement have automated checks; touch-pointer event handling has a DOM test. Physical touchscreen devices and Safari/iOS remain unverified. The live Booqable catalog and calendar popup were checked. No payment or message delivery was tested.
...
Contact buttons prepare WhatsApp or email drafts for the customer to review and send. If the app does not open, use the draft link displayed below the form. They do not send messages automatically.
